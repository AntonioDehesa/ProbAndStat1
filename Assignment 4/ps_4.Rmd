...
```{r}
y_bar <- mean(dat.both$den.log)
x_bar <- mean(dat.both$pop.log)
m<- (mean(dat.both$den.log*dat.both$pop.log) - y_bar*x_bar)/(mean(dat.both$pop.log^2) - x_bar^2)
b <- mean(dat.both$den.log - m*dat.both$pop.log)
f <- function(m,x,b){
  return(m*x+b)
}
g2 <- g + geom_abline(slope=m,intercept = b)+
  expand_limits(x = 5, y = 15)# Plot of y = mx + b
g2
```